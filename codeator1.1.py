    ...
    for i, line in enumerate(lines):
        ...
        if match:
            ...
            if name in items_to_remove:
                ...
                continue  # skip this def/class line

        if skip_block:
            # Keep skipping until indentation level drops back
            current_indent = len(line) - len(line.lstrip())
            if line.strip() == "":  # allow blank lines inside block
                continue
...
def export_cleaned():
    ...
    if not selected_to_remove:
        messagebox.showinfo("No Selection", "No defs/classes selected for removal.")
        return
...
# --- UI SETUP ---
root = tk.Tk()
root.title("🧠 Codeator - Interactive Python Code Analyzer")
root.geometry("950x900")
root.configure(bg="#f9f9f9")

title_label = tk.Label(root, text="🧠 Codeator 2.0", font=("Arial", 22, "bold"), bg="#f9f9f9", fg="#222")
...
output_label = tk.Label(root, text="Analysis Result (click to mark for deletion):",
